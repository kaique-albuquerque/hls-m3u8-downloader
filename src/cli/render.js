...
/** Imprime o erro da análise: mensagem amigável + detalhe técnico (opcional). */
export function printAnalysisError(io, err) {
  const message = err?.message || String(err || 'erro desconhecido');
  io.error(`\n[ERRO] ${message}`);
  if (err?.status === 403) {
    io.error('A URL foi recusada pelo servidor. Ela pode ter expirado ou o servidor exige headers HTTP de navegador.');
  } else if (err?.needsAuth) {
    io.error('O conteudo parece exigir login. Use --cookies <arquivo> ou --cookies-from-browser <navegador>.');
  }
}
